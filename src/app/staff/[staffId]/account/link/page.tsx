...
import { StaffAccountLinkPage } from "@/features/staff-accounts/components/staff-account-link-page";
import { staffAccountService } from "@/lib/api/staff-accounts";
import { staffService } from "@/lib/api/staff";
import { currentSessionRole } from "@/lib/current-user";
import { tenantContext } from "@/lib/tenant-context";

export default async function StaffAccountLinkRoute({ params }: { params: Promise<{ staffId: string }> }) {
  if (!hasPermission(currentSessionRole, "hr.manage")) redirect("/unauthorized");
  const { staffId } = await params;
  if (!isValidStaffId(staffId)) return <AppShell><StaffProfileNotFound message="The staff account link URL is not valid." /></AppShell>;
  ...
  return <AppShell><StaffAccountLinkPage accountView={accountView} staff={staff} /></AppShell>;
}

...
import { StaffDocumentUploadPage } from "@/features/staff-documents/components/staff-document-upload-page";
import { staffDocumentService } from "@/lib/api/staff-documents";
import { staffService } from "@/lib/api/staff";
import { currentSessionRole } from "@/lib/current-user";
import { tenantContext } from "@/lib/tenant-context";

export default async function NewStaffDocumentPage({ params }: { params: Promise<{ staffId: string }> }) {
  if (!hasPermission(currentSessionRole, "hr.manage")) redirect("/unauthorized");
  const { staffId } = await params;
  if (!isValidStaffId(staffId)) {
    return <AppShell><StaffProfileNotFound message="The staff document upload URL is not valid." /></AppShell>;
  }
  ...
  return (
    <AppShell>
      <StaffDocumentUploadPage definitions={documentsResponse.definitions} staff={staff} />
    </AppShell>
  );
}
